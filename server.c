	...
	while(deathflag == 1){
		...
		if(sscanf(line,"%9s %101s", command, account) == 2){
			if(strcmp(command, "deposit") == 0 || strcmp(command, "withdraw") == 0){
				...
				if(send(sd, "You must be in a session to withdraw or deposit.", 49 , 0) == -1){
					perror("send");
				}	
				continue;
			}	
			...
			if(strcmp(command, "deposit") == 0){
				deposit(act, munni);
				if(send(sd, "Deposited funds.", 17 , 0) == -1){
					perror("send");
				}	
			}
			...
			//Withdraw
				...
				withdraw(act, munni);
				if(send(sd, "Withdrew funds. Thank you for your generous donation. ", 55 , 0) == -1){
					perror("send");
				}	
			}
			
			else{
				if(send(sd, "Unspecified error...", 21 , 0) == -1){
					perror("send");
				}	
			}
			sem_wait(welcome);
			...
			if(insesh == 1){
				...
				if(send(sd, "Active customer session: cannot create or serve new account.", 61 , 0) == -1){
					perror("send");
				}
			...
			if(strcmp(command, "create") == 0){
				for(i = 0; i < 20; i++){
					if((p[i].name)[0] == '\0'){//We need to init all SHM to 0
						...
						if(send(sd, "Account name already exists.", 29 , 0) == -1){
							perror("send");
						}
						//sem_post(writeo);
						//sem_post(reado);
				...
				if(i == 20){
					...
					if(send(sd, "Could not find account.", 24 , 0) == -1){
						perror("send");
					}
				}
			}
			...
			if(i == 20){
				...
				if(send(sd, "Error, bank full.", 18,0) == -1){
					perror("send");
				}
			}
		}
		...
		//query, end, quit
			...
			if(insesh == 0){
				...
				//Send something like 'you must be in a session to use this operation.'
					if(send(sd,"You must be in a session to use this operation.", 48, 0)==0){
						perror("send");
					}
				}
				continue;
			...
			//end
				...
				if(send(sd,"Client session ended. You may now create another account, or be served.", 72, 0) == -1){
					perror("send");
				}
			...
			//quit
				...
				if(insesh == 1){
					pthread_mutex_unlock(&(act->lock));
					if(send(sd,"Client session ended. Goodbye.", 31 , 0) == -1){
						perror("send");
					}
				}
				else{
					if(send(sd,"Goodbye.", 9 , 0) == -1){
						perror("send");
					}
				}
				raise(SIGINT);
				...
				if(send(sd, "Unspecified error, eq...", 25 , 0) == -1){
					perror("send");
				}	
			}
			sem_wait(welcome);
